...
export class Pathfinder{
  ...
  getNeighbors (x, y) {
    ...
    if (!this.blocked(x + 1, y)) neighbors.push({x:x + 1,y:y, s:1});
    if (!this.blocked(x - 1, y)) neighbors.push({x:x - 1,y:y, s:1});
    if (!this.blocked(x, y + 1)) neighbors.push({x:x,y:y + 1, s:1});
    if (!this.blocked(x, y - 1)) neighbors.push({x:x,y:y - 1, s:1});


    if (!this.blocked(x + 1, y + 1) && (!this.blocked(x + 1, y) || !this.blocked(x, y + 1))) neighbors.push({x:x + 1,y:y + 1, s:2});
    if (!this.blocked(x - 1, y - 1) && (!this.blocked(x - 1, y) || !this.blocked(x, y - 1))) neighbors.push({x:x - 1,y:y - 1, s:2});
    if (!this.blocked(x + 1, y - 1) && (!this.blocked(x + 1, y) || !this.blocked(x, y - 1))) neighbors.push({x:x + 1,y:y - 1, s:2});
    if (!this.blocked(x - 1, y + 1) && (!this.blocked(x - 1, y) || !this.blocked(x, y + 1))) neighbors.push({x:x - 1,y:y + 1, s:2});
  ...
  findPath (xC, yC, xT, yT, maxSteps) {
    ...
    while (this.open.length !== 0 && stepCount < 10000) {
      stepCount++
      ...
      for (i = 0; i < neighbors.length; i++) {
        ...
        stepCost = current.g + neighbors.s;
